...
export function createMemoryTool(memoryStore: MemoryStore): ToolDefinition {
  return {
    name: 'memory',
    description: '管理跨会话记忆。action: save（保存）| list（列表）| search（搜索）| read（读取）| delete（删除）。save 时若用相同 name 则更新同一文件；检测到疑似同主题记忆会拒绝新建并提示——同主题请用其 name 更新，确属新主题再传 force=true。',
    parameters: {
      type: 'object',
      properties: {
        action: { type: 'string', enum: ['save', 'list', 'search', 'read', 'delete'] },
        name: { type: 'string', description: '记忆名称（save 时必填）；用已有 name 即更新该记忆' },
        ...
        force: { type: 'boolean', description: '跳过重复检测强制新建（确认与已有记忆不同主题时使用）' },
